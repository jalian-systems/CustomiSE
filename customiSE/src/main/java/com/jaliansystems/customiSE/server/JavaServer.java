...
public class JavaServer extends NanoHTTPD {
    ...
    static {
        try {
            ...
            hasCapabilities.put("platform", "custom-selenium-driver");
    ...
    private void initCapabilities() {
        ...
        capabilities.put("platform", "custom-selenium-driver");
    ...
    public Object invoke(Route route, JSONObject query, JSONObject uriParams, Session session, IWindow window,
    ...
    public Response createSession(JSONObject query, JSONObject uriParams) {
        ...
        DeviceType t = DeviceType.COOKED;
        if (capabilities.getBoolean("nativeEvents"))
            t = DeviceType.RAW;
        Logger.getLogger(JavaServer.class.getName()).info("Creating device with type: " + t);
        IJavaAgent agent;
        String agentProperty = System.getProperty("custom.selenium.driver.agent");
        if (agentProperty == null) {
            throw new JavaAgentException(
                    "You need to provide the class that implements IJavaAgent in custom.selenium.driver.agent property", null);
        }
        try {
            agent = (IJavaAgent) Class.forName(agentProperty).newInstance();
        } catch (InstantiationException | IllegalAccessException | ClassNotFoundException e1) {
            throw new JavaAgentException(e1.getMessage(), e1);
        }
        ...
        try {
            Response r = nanoHTTPDnewFixedLengthResponse(Status.REDIRECT, MIME_HTML, "Session Created: " + session.getID());
    ...
    public JSONObject getWindowSize(JSONObject query, JSONObject uriParams, Session session, IWindow window) {
        return new JSONObject(window.getSize());
    }

    public JSONObject getWindowPosition(JSONObject query, JSONObject uriParams, Session session, IWindow window) {
        return new JSONObject(window.getLocation());
    }

    public void setWindowSize(JSONObject query, JSONObject uriParams, Session session, IWindow window) {
        checkRequiredArguments(query, "width", "height");
        window.setSize(query.getInt("width"), query.getInt("height"));
    }

    public void setWindowPosition(JSONObject query, JSONObject uriParams, Session session, IWindow window) {
        checkRequiredArguments(query, "x", "y");
        window.setLocation(query.getInt("x"), query.getInt("y"));
    }

    public void maximizeWindow(JSONObject query, JSONObject uriParams, Session session, IWindow window) {
        window.maximize();
    }
    ...
    public Object execute(JSONObject query, JSONObject uriParams, Session session) {
        ...
        return session.execute(query.getString("script"), query.getJSONArray("args"), session, ExecuteMode.SYNC);
    }

    public Object executeAsync(JSONObject query, JSONObject uriParams, Session session) {
        checkRequiredArguments(query, "script", "args");
        return session.execute(query.getString("script"), query.getJSONArray("args"), session, ExecuteMode.ASYNC);
    }
